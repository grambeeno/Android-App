...
/**
 * DataManager: Manages calls for LokaliteDB and HTTPConnect
...
public class DataManager {
	...
	public ArrayList<OrganizationDTO> getOrganizationsList(String cat, String url, TypeReference<ArrayList<OrganizationDTO>> o){
		...
		// Only go to server if it's been over 30 minutes since Orgs was updated
		if (lastUpdated == -1 || lastUpdated == 0 || (currentTime.getTime() - lastUpdated) >= 10800000)
		{
			//Connecting to server
	 ...
	 * only if it's been at least 10 minutes since the last time the table has been updated
	 * @param url
	 * @param o
	 */
	public void updateDBEvents(String url, TypeReference<ArrayList<EventDTO>> o, String cat, boolean goToServer){
		...
		//Go to server and update DB if it has been 10 minutes
		if (storedTime == -1 || storedTime == 0 || (currentTime.getTime() - storedTime) >= 3600000 || goToServer == true){
			Log.i(TAG, "DataManager: going to server");

			//Go to server and update DB
			try{	
