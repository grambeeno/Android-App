...
public class ICUPDb extends SQLiteOpenHelper {
    ...
    private final Context mCtx;
        ...
        @Override
        /**
         * Called by the Android OS when ICUPDb is created
    ...
    public ArrayList<OrganizationDTO> fetchAllOrganizations(String url){
    	...
    	Log.i("ICUPDb", "category is: "+ category);
    	Cursor mCursor;
    	...
    	if (mCursor != null && 	mCursor.moveToFirst()) {
    		...
    		for(int i = 0; i< count; i++){
				...
				try {
					...
					Log.i("ICUPDb", "IOException when getting OrgDTO's from database from fetchAllOrganizations");
					e.printStackTrace();
				}
    	...
    	if (mCursor != null && 	mCursor.moveToFirst()) {
    		...
    		for(int i = 0; i< count; i++){
				...
				try {
					...
					Log.i("ICUPDb", "IOException when getting EventDTO's from database in fetchAllEvents()");
					e.printStackTrace();
				}
    ...
    public void databaseClean(){
    	...
    	if (mCursor != null && 	mCursor.moveToFirst()) {
    		int count = mCursor.getCount();
    		Log.i("ICUPDb", "Number of events removed DBClean: " + String.valueOf(count));
    		for(int i = 0; i< count; i++){
    			Log.i("ICUPDb", "Current time: " + start.getTime() + "End Date: " + mCursor.getLong(mCursor.getColumnIndex(KEY_END_DATE)) 
    ...
    public ArrayList<EventDTO> searchEvents(String category,String name,String orgName, Date start, Date end) {
    	...
    	if (mCursor != null && 	mCursor.moveToFirst()) {
    		...
    		for(int i = 0; i< count; i++){
    			JSON = mCursor.getString(mCursor.getColumnIndex(KEY_JSON_DTO));
    			Log.i("ICUPDb", "Events found within date range =" + String.valueOf(count));
				try {

					events.add(HTTPConnect.getObject(JSON, EventDTO.class));
				} catch (IOException e) {
					Log.i("ICUPDb", "IOException when getting EventDTO's from database from searchEvents()");
					e.printStackTrace();
				}
    	...
    	if (mCursor != null && 	mCursor.moveToFirst()) {
    		...
    		for(int i = 0; i< count; i++){
				...
				try {
					...
					Log.i("ICUPDb", "IOException when getting EventDTO's from database from searchOrgs");
					e.printStackTrace();
				}
	...
	{		Log.i("ICUPDb", "isEmpty() category"+ t);
	Cursor orgCursor;
	if(t.matches("All")){
	...
	Log.i("ICUPDb", "isEmpty() number of rows"+ String.valueOf(orgCursor.getCount()));
